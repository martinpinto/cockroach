...
type queueConfig struct {
	...
	// If this value is non-zero, needsLease MUST be true because the last
	// processed time is stored and replicated for a range, not any one
	// specific replica.
	// TODO(spencer): support per-replica last processed times.
...
func newBaseQueue(
	...
	// Cannot specify minInterval > 0 if the queue isn't limited to leaseholders.
	if cfg.minInterval > 0 && !cfg.needsLease {
		panic(fmt.Sprintf("queue %s specifies minInterval=%s but also "+
			"doesn't require the range lease", name, cfg.minInterval))
	}
...
func (bq *baseQueue) MaybeAdd(repl *Replica, now hlc.Timestamp) {
	...
	lastProcessedPriority := float64(1)
	if bq.needsLease {
		...
		if bq.minInterval > 0 && !repl.store.cfg.TestingKnobs.DisableLastProcessedCheck {
			...
			lpTS := qs.GetLastProcessed(bq.name)
			if !shouldQueueAgain(now, lpTS, bq.minInterval) {
			...
			if lpTS != hlc.ZeroTimestamp {
				lastProcessedPriority = float64(now.GoTime().Sub(lpTS.GoTime()).Nanoseconds()) /
					float64(bq.minInterval.Nanoseconds())
			}
		}
	}

	should, priority := bq.impl.shouldQueue(ctx, now, repl, cfg)
	priority *= lastProcessedPriority
